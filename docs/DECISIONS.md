...
- The normal steady-state deployment prebuild should generate the Prisma client only. For the one-time Hostinger rollout of the additive homepage migration and permission seed, `prebuild` temporarily generates the client, runs `scripts/hostinger-prisma-migrate.mjs`, and seeds. The wrapper dynamically copies Prisma's installed schema engine to the system temporary directory, requests executable mode `0755`, verifies `--version`, and supplies it through `PRISMA_SCHEMA_ENGINE_BINARY` to `prisma migrate deploy`. Keep this temporary sequence until the client confirms that deployment succeeded; never use `prisma migrate reset` or another destructive database command.
